...
class HardModeController2 extends GetxController {
  ...
  RxString currentExpression = RxString('');
  ...
  HardModeController2({required List<CameraDescription> cameras}) {
    _cameras = cameras;
  }
  ...
  void startStage() {
    ...
    _updateMessage();
    _startCountdown();
    Logger().e("current expression: ${currentExpressions}");
  }

  void _selectRandomExpressions() {
    _currentExpressions.value =
        (allExpressions.toList()..shuffle()).take(2).toList();
  ...
  void _updateMessage() {
    _message.value =
        'FACE ${_currentExpressionIndex.value + 1}: ${_currentExpressions[_currentExpressionIndex.value]}';
    currentExpression.value =
        _currentExpressions.value[_currentExpressionIndex.value];
    Logger().e('_currentExpression.value: ${currentExpression.value}');
    if (currentExpression.value == 'TEMP1') {
      _currentExpressions[_currentExpressionIndex.value] = 'FROWN';
    }
  }
  ...
  Future<void> _captureAndAnalyze() async {
    try {
      ...
      bool isExpressionCorrect = _validateExpression(
          face, _currentExpressions[_currentExpressionIndex.value]);
      ...
      if (isExpressionCorrect) {
        ...
        if (userId != null) {
          await saveStatistics(
              userId,
              _currentExpressions[_currentExpressionIndex.value],
              1,
              0); // 성공 결과 저장
        }
        _showFeedback(true);
  ...
  Future<void> saveStatistics(String userId, String expressionType,
      int successCount, int failCount) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
  ...
  void _finishStage() {
    ...
    if (_correctExpressions.value > 0) {
      Get.off(() => StageClear2(result: result), arguments: result);
    } else {
      Get.off(() => StageFail2(result: result), arguments: result);
    }
  }
